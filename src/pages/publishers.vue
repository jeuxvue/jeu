<template>
  ...
  <div v-else class="flex flex-col max-w-screen-lg m-auto xl:max-w-screen-xl">
    <YoBox class="flex items-end w-full gap-4 mt-8">
      <InputWrap class="flex-grow" title="Search for a publisher">
  ...
  <div class="flex justify-center w-full mt-10">
    <div class="flex flex-row w-full max-w-screen-lg gap-12 xl:max-w-screen-xl">
      <CreatorsGallery :creators="creators" :creator-data="data" creator-type="publisher" @fetch-next="fetchNext" />
    </div>
  </div>
</template>
...
import { getPublishers } from '@/api/creators'
...
onMounted(async() => {
  data.value = await getPublishers(currentSearchParams.value)
